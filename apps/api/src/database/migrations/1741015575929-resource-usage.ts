...
export class ResourceUsage1741015575929 implements MigrationInterface {
    name = 'ResourceUsage1741015575929'
    ...
    END) STORED NOT NULL`);
        ...
        await queryRunner.query(`CREATE VIEW "resource_computed_view" AS SELECT "resource"."id" AS "id", COALESCE(SUM("usage"."usageInMinutes"), 0) AS "totalUsageMinutes" FROM "resource" "resource" LEFT JOIN "resource_usage" "usage" ON "usage"."resourceId" = "resource"."id" GROUP BY "resource"."id"`);
        await queryRunner.query(`INSERT INTO "typeorm_metadata"("database", "schema", "table", "type", "name", "value") VALUES (DEFAULT, $1, DEFAULT, $2, $3, $4)`, ["public","VIEW","resource_computed_view","SELECT \"resource\".\"id\" AS \"id\", COALESCE(SUM(\"usage\".\"usageInMinutes\"), 0) AS \"totalUsageMinutes\" FROM \"resource\" \"resource\" LEFT JOIN \"resource_usage\" \"usage\" ON \"usage\".\"resourceId\" = \"resource\".\"id\" GROUP BY \"resource\".\"id\""]);
    }
